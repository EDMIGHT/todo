...
export const Checkbox: FC<CheckboxProps> = ({ id = 'check', className, ...rest }) => {
  return (
    <div className='inline-flex items-center'>
      <label className='relative flex items-center rounded-full cursor-pointer' htmlFor={id}>
        <input
          ...
          type='checkbox'
          className={cn(
            "before:content[''] peer relative h-5 w-5 cursor-pointer appearance-none rounded-md border border-border transition-all checked:border-primary checked:bg-primary",
            className
          )}
          {...rest}
        />
